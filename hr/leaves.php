...
<div class="container-fluid mt-4">
    ...
    <div class="tab-content">
                    ...
                    <tbody>
                        ...
                        if(mysqli_num_rows($q) > 0) {
                            while($row = mysqli_fetch_assoc($q)) {
                                ...
                                echo "<tr>
                                    <td>{$row['first_name']}</td>
                                    <td><span class='badge bg-success'>" . ucfirst($row['type']) . "</span></td>
                                    <td>{$row['start_date']}</td>
                                    <td>{$row['end_date']}</td>
                                    <td>{$days}</td>
                                    <td>" . date('d M Y', strtotime($row['applied_at'])) . "</td>
                            ...
                            echo "<tr><td colspan='6' class='text-center text-muted'>No approved leave applications</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
        ...
        <div id="rejected" class="tab-pane fade">
            <div class="card p-4">
                <h5>Rejected Leave Applications</h5>
                <table class="table table-striped mt-3">
                    <thead>
                        <tr>
                            <th>Employee</th>
                    ...
                    <tbody>
                        ...
                        if(mysqli_num_rows($q) > 0) {
                            while($row = mysqli_fetch_assoc($q)) {
                                ...
                                echo "<tr>
                                    <td>{$row['first_name']}</td>
                                    <td><span class='badge bg-danger'>" . ucfirst($row['type']) . "</span></td>
                                    <td>{$row['start_date']}</td>
                                    <td>{$row['end_date']}</td>
                                    <td>{$days}</td>
                                    <td>" . date('d M Y', strtotime($row['applied_at'])) . "</td>
                            ...
                            echo "<tr><td colspan='6' class='text-center text-muted'>No rejected leave applications</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
